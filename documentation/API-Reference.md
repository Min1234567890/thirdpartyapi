...
2. Check CSN not already assigned in JCMS (CSN_No)
3. Derive 52-char DevicePIN from CSN (SDK §1.4.2)
4. Check DevicePIN not already in use (TUser)
5. Look up cardholder in JCMS (JC_CARDDTL)
6. INSERT TTask (Type=159) with DevicePIN → XAgent → VPX
   VPX enrols biometric, XAgent creates TUser with PIN
7. Poll TTask until Status=3 (max 30s)
8. Read TUser.Id + TUser.PIN (PIN generated by VPX/XAgent)
9. INSERT TUser_Info (name, number)
10. UPDATE JC_CARDDTL SET CSN_No = csn
11. INSERT Enrolment_Log
12. Return SUCCESS with PIN from TUser
```
...
The API derives only the **DevicePIN** from the CSN. The **PIN** (decimal) is
generated by the VPX/XAgent and stored in `TUser.PIN` — the API reads it back
after enrolment completes.

```
8-hex CSN:       "9E230EAA"         ← from QR code
52-char DP:      "03224F1187..."    ← API derives via Wiegand pipeline (SDK §1.4.2)
Decimal PIN:     "2653097642"       ← VPX/XAgent generates and stores in TUser.PIN
```
