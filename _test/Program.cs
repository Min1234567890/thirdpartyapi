...
TestEnrolment(string hvprx, string smartPass, string jcmsOracle, bool useOracle, int vpxDevId)
{
    ...
    // Generate CSN and derive DevicePIN (PIN will be set by VPX/XAgent)
    string csn = GenerateTestCSN();
    string dp = CSNtoDevicePIN(csn);
    r.Add(($"CSN={csn} DP={dp.Substring(0,14)}...", "SUCCESS", "SUCCESS", csn, "", dp, null));

    // CSN duplicate check in JCMS
    try
    {
        using var conn = new SqlConnection(smartPass); conn.Open();
        using var cmd = new SqlCommand("SELECT COUNT(1) FROM JC_CARDDTL WHERE CSN_No = @csn", conn);
        cmd.Parameters.AddWithValue("@csn", csn);
        if (Convert.ToInt32(cmd.ExecuteScalar()) > 0)
        { r.Add(("CSN duplicate check", "PASS", "FAILURE", csn, "", dp, "CSN already in JCMS")); return r; }
    }
    catch (Exception ex) { r.Add(("CSN check", "PASS", "FAILURE", csn, "", dp, ex.Message)); return r; }
    r.Add(("CSN not in use", "PASS", "PASS", csn, "", dp, null));
    ...
    r.Add(("DevicePIN not in use", "PASS", "PASS", csn, "", dp, null));

    // SIMULATE VPX: insert TUser (what XAgent does — VPX/XAgent sets the PIN)
    // In production, PIN is generated by VPX/XAgent. For test, we provide it.
    int userId = 0;
    string pin = Convert.ToUInt32(csn, 16).ToString();  // PIN = decimal CSN (what XAgent would store)
    try
    {
        using var conn = new SqlConnection(hvprx); conn.Open();
        using var cmd = new SqlCommand(@"
    ...
    // Read back PIN from TUser (in production, API reads this after XAgent creates the row)
    string actualPin = pin;
    try
    {
        using var conn = new SqlConnection(hvprx); conn.Open();
        using var cmd = new SqlCommand("SELECT PIN FROM TUser WHERE Id = @id", conn);
        cmd.Parameters.AddWithValue("@id", userId);
        var p = cmd.ExecuteScalar();
        if (p != null) actualPin = p.ToString();
    }
    catch { }
    r.Add(($"VPX: TUser.Id={userId} PIN={actualPin}", "SUCCESS", "SUCCESS", csn, actualPin, dp, null));
